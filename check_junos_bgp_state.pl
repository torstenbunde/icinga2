...
# Changes in 0.2:
#  - added AS Info to output
#
# 2023-02-20, Torsten Bunde (github@torstenbunde.de)
# - Added "maxrepetitions => 1" for get_table in lines 119, 120 and 121
#   because of empty results from some devices (Model: mx960)
#
#Example syntax:
#
...
my $bgp_peer_as_oid = '1.3.6.1.4.1.2636.5.1.1.2.1.1.1.13';
#BGP states
...
# Do the SNMP queries
my $result_bgp_state = $session->get_table(Baseoid => $bgp_peer_state_oid, maxrepetitions => 1 );
my $result_bgp_admin_status = $session->get_table(Baseoid => $bgp_peer_admin_status_oid, maxrepetitions => 1 );
my $result_bgp_remote_as = $session->get_table(Baseoid => $bgp_peer_as_oid, maxrepetitions => 1 );
...
my @bgp_peer_as;
my $keyas;
foreach my $keyas ( sort (keys %$result_bgp_remote_as)) {
	$bgp_peer_as[$i] = $$result_bgp_remote_as{$keyas};
	$i++;
}
...
#	print $$result_bgp_state{$key};
	...
	if ( ($$result_bgp_state{$key}) lt 6) {
		...
		if ( $bgp_admin_status[$i] eq "1" ) {
	        	$warning="yes";
			$output = "Peer ";
                	$output_OK .= "Peer: $peer_ip"."(AS".$bgp_peer_as[$i].") ";
                	$output_OK .= "Shutdown\n";
		}
		# Not manually shutdown/disabled
		else {	
			$critical = "yes";
                	$output .= "Peer: $peer_ip "."(AS".$bgp_peer_as[$i].") ";
...
# Print the results
if ($warning eq "yes") {
        print $output," : WARNING | bgp_peers=$establised_bgp_peers\n";
	print $output_OK;
        exit $ERRORS{"WARNING"};
}
